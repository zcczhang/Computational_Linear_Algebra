...
\max _{\|x\|_{\infty}=1} \left\{ \max _{1 \leq i \leq n}\left\{\left|\sum_{i=1}^{n}A_{i} x_{j} \right|\right\}\leqslant \max _{\| x \|_{\infty}=1} \max _{1 \leq i \leq n}\sum_{j=1}^{n}\left|A_{i}\right|\left|x_{j}\right|\right.\\ 
& \leqslant \max _{1 \leq j \leq n} \sum_{j=1}^{n}\| A_{ij} \| = \text{max row sum}\end{aligned}
$$
