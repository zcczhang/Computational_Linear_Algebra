...
> After around 6 iterations, it will become converge. The convergence order is $1$(linear), and rate is around $0$.
...
> The convergence order is $1$(linear), and rate is around $0.5$. It is different from d) since:<br>
$\because f^{\prime}(r)\neq 0$ for both root $\pi$ and $\pi/2$<br>
$\therefore f(r)=0=f(x_{i})+f^{\prime}(x_{i})(r-x_{i})$<br>
$\therefore f(r)=f(x_{i})+f^{\prime}(x_{i})(r-x_{i})$<br>
$\therefore 0=\frac{f(x_{i})}{f^{\prime}(x_{i})}+\frac{f^{\prime}(x_{i})(r-x_{i})}{f^{\prime}(x_{i})}$<br>
$\therefore x_{i}-\frac{f(x_{i})}{f^{\prime}(x_{i})}-r =\frac{(r-x_{i})^{2}}{2} \frac{f^{\prime \prime}(c_{i})}{f^{\prime}(x_{i})}$<br>
$\therefore x_{i+1}-r =e_{i}^{2} \frac{f^{\prime \prime}(c_{i})}{2 f^{\prime}(x_{i})}$<br>
$\therefore e_{i+1} =e_{i}^{2}|\frac{f^{\prime \prime}(c_{i})}{2 f^{\prime}(x_{i})}|$<br>
...
$\therefore 34$ iterations of the bisection method are necessary to find the accurate solution to ten correct decimal places.
...
> The slop is 0.984247752914 approximately.
...
```{r}
secant.method <- function(f, x0, x1, tol = 0.5e-10, n = 40) {
  history = rep(NA, n) 
  for (i in 1:n) {
