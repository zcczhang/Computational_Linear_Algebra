...
W <- spMatrix(nrow =12, ncol=12, 
              i=c(2:5,8:11,1:4,7:10),
              j=c(1:4,7:10,2:5,8:11),
              x=c(c(1,2,1,1),c(1,2,1,1),c(1,2,1,1),c(1,2,1,1)))
a = 1
W[5,1] <- 2
W[1,5] <- 2
W[6,4] <- 2
...
```{r}
# Your code here
FDDeriv = function(f,delta=.000001){ function(x){(f(x+delta) - f(x-delta))/(2*delta)}
}
g=FDDeriv(sg)
...
secant = function(f, a, b, tol = 1e-05, maxiters = 12) { history = rep(NA, maxiters)
history[1] = a
history[2] = b
for (k in 3:maxiters) {
x1 = history[k-2]
...
# Your code here
library(readr)
AirlineStockPrices <- read_csv("AirlineStockPrices.csv",
...
*Include your answer in text here.*